...
if (isActionAccessible($guid, $connection2, '/modules/Visual Assessment/guides_manage.php') == false) {
    ...
    if ($highestAction == false) { echo "<div class='error'>";
        ...
        echo "<div class='trailHead'><a href='".$_SESSION[$guid]['absoluteURL']."'>".__($guid, 'Home')."</a> > <a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.getModuleName($_GET['q']).'/'.getModuleEntry($_GET['q'], $connection2, $guid)."'>".__($guid, getModuleName($_GET['q']))."</a> > </div><div class='trailEnd'>".__($guid, 'Manage Visual Assessment Guides').'</div>';
        echo '</div>';
		...
		if ($result->rowCount() < 1) {
			...
			while ($row = $resultPage->fetch()) {
                ...
                if ($highestAction == 'Manage Assessment Guides_all') {
                    echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module'].'/guides_manage_edit.php&visualAssessmentGuideID='.$row['visualAssessmentGuideID']."&search=$search&filter2=$filter2'><img title='".__($guid, 'Edit')."' src='./themes/".$_SESSION[$guid]['gibbonThemeName']."/img/config.png'/></a> ";
                    echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module'].'/guides_manage_delete.php&visualAssessmentGuideID='.$row['visualAssessmentGuideID']."&search=$search&filter2=$filter2'><img title='".__($guid, 'Delete')."' src='./themes/".$_SESSION[$guid]['gibbonThemeName']."/img/garbage.png'/></a> ";
                    echo "<a href='".$_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.$_SESSION[$guid]['module'].'/guides_manage_view.php&visualAssessmentGuideID='.$row['visualAssessmentGuideID']."&sidebar=false&search=$search&filter2=$filter2'><img style='margin-left: 3px' title='".__($guid, 'View')."' src='./themes/".$_SESSION[$guid]['gibbonThemeName']."/img/plus.png'/></a>";
                ...
                if ($row['description'] != '') {
                    echo "<a title='".__($guid, 'View Description')."' class='show_hide-$count' onclick='false' href='#'><img style='padding-left: 3px' src='".$_SESSION[$guid]['absoluteURL']."/themes/Default/img/page_down.png' alt='".__($guid, 'Show Comment')."' onclick='return false;' /></a>";
                }
                echo '</td>';
                echo '</tr>';
                if ($row['description'] != '') {
