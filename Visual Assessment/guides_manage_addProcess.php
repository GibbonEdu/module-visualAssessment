...
$URLSuccess=$_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_POST["address"]) . "/guides_manage_edit.php&&search=$search&filter2=$filter2" ;
		...
		if ($highestAction!="Manage Assessment Guides_all" AND $highestAction!="Manage Assessment Guides_myDepartments") {
			...
			if ($scope=="" OR ($scope=="Learning Area" AND $gibbonDepartmentID=="") OR $name=="" OR $active=="") {
				...
				if ($AI=="") {
					...
					//Success 0
					$URL=$URLSuccess . "&addReturn=success0&visualAssessmentGuideID=$AI" ;
					header("Location: {$URL}");
				}
			}
		}
